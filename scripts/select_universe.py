...
KEEP = ["symbol", "tier", "risk", "cmc_rank", "pair_address", "token_address",
        "bsc_contract", "cmc_id", "liq_usd", "vol_h24", "turnover", "vol_proxy",
        "needs_verification", "dex", "quote", "name"]
...
def main() -> None:
    ...
    print(f"selected (risk-tiered by CMC rank): {len(chosen)}  ->  {args.out}\n")
    print(f"  {'tier':7} {'symbol':12} {'rank':>5} {'liq_usd':>13} {'vol_h24':>13} {'turnover':>8}")
    for r in chosen:
        print(f"  {r['tier']:7} {r['symbol']:12} {str(r.get('cmc_rank')):>5} "
              f"{r['liq_usd']:>13,.0f} {r.get('vol_h24',0):>13,.0f} {r.get('turnover',0):>8.2f}")
