...
a frame disconnected from beating-the-rung-0-RULE-under-the-DQ-gate. This module builds that header
with live state pulled from the latest published bundle, so a consult can never be framed as a bare
sub-problem — the exact orchestration failure that lost exp1→exp5 a day optimizing a proxy that
stopped paying profit.
...
GOAL = ("a self-custody RL trading agent that is PROFITABLE and risk-managed on live PnL "
        ...
        "never a destination.")
SUCCESS_METRIC = ("on HELD-OUT data, the policy must (1) SURVIVE the DQ gate (maxDD < ~30%) and "
                  "(2) BEAT the rung-0 RULE — reported PER REGIME (bull/bear/flat). Buy&Hold and "
                  "Random are COMPUTED and REPORTED references but are NEVER binding (DIRECTION "
                  "RESET 2026-06-15: requiring 'beat Buy&Hold' rewards holding-everything). This is "
                  "honest_gate() in scripts/train_event.py. A reward proxy is legitimate ONLY with "
                  "evidence that proxy => this metric.")
...
def north_star_header(ask: str, diag: dict | None = None, *, split: str = "?") -> str:
    ...
    return (
        ...
        "beating-the-rung-0-RULE-under-the-DQ-gate, SOUND THE DRIFT ALARM and stop — do not "
        "optimize a proxy locally.")
